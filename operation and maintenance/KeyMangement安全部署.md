...
- Key Mangement使用的数据库（包括mysql和mongo DB）要设置定时备份，主数据库定时同步到从数据库。

- 数据库内容要定时进行备份并保存至本地

- 本地保存的数据库备份需存储在加密硬盘中，并且保存的数据库位置禁止联网。防止计算机被入侵造成的数据泄露。
...
ogappend=true------写日志的模式：设置为true为追加。默认是覆盖。如果未指定此设置，启动时MongoDB的将覆盖现有的日志文件。
